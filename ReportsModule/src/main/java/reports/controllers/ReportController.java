...
public class ReportController {
	...
	@RequestMapping(value = "/project/{id}", method = RequestMethod.GET, produces = "applicaiton/json")
	public ResponseEntity<Object> Report(@PathVariable("id") Long id) {
		if (id == null)
			return new ResponseEntity(new ResponseModel("Error while fetching data"), HttpStatus.BAD_REQUEST);
		try {
			
			//ProjectModel project = repository.GetById(id).getBody();

			//result.setProject(project);
			
			return new ResponseEntity<>(repository.GetById(id).getBody(), HttpStatus.OK);
	...
	public ResponseEntity<String> tasksById(@RequestParam(value = "projectId") Long projectId,
	...
	public ResponseEntity<FinishedTaskGroupedTotal> finishedTasksGroupedBy(
			@RequestParam(value = "taskStatus") Long status) {
